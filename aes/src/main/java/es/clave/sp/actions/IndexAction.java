...
public class IndexAction extends AbstractSPServlet {
    ...
    private boolean movilCheck = false;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (acceptsHttpRedirect()) {
			...
			System.out.print("Llamada al metodo index get " + date + "\n");
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		movilCheck = Boolean.parseBoolean(request.getParameter("movilCheck"));
		...
		eidasloa = "http://eidas.europa.eu/LoA/substantial";
        ...
        if (movilCheck) {
        	reqAttrMapBuilder.put(new AttributeDefinition.Builder<String>().nameUri("http://es.minhafp.clave/CLVMOVILIdP")
        			...
        			.required(false)
        			.uniqueIdentifier(true)
        			.xmlType("http://www.w3.org/2001/XMLSchema", "CLVMOVILIdPType", "cl")
        			.attributeValueMarshaller(new StringAttributeValueMarshaller())
        			.build());
        }
        ...
        try {
            ...
            request.setAttribute("Title","Could not generate token for Saml Request");
		...
		dispatcher = request.getRequestDispatcher("/redirect.jsp");
		dispatcher.forward(request, response);
    }
